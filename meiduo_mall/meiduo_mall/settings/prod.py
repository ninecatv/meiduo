...
SECRET_KEY = 'ie##!1o64t-(bxm__sd2e4mol-3ca4k+yl^nzb)pqbcmai0flz'
...
INSTALLED_APPS = [
    ...
    'oauth.apps.OauthConfig',  # 注册用户qq登陆的子应用
...
REST_FRAMEWORK = {
    ...
    'DEFAULT_AUTHENTICATION_CLASSES': (  # 配置全局认证类(验证登陆用户是不是本网站用户)
        'rest_framework_jwt.authentication.JSONWebTokenAuthentication',  # JWT默认认证方案
        'rest_framework.authentication.SessionAuthentication',  # session认证
        'rest_framework.authentication.BasicAuthentication',  # 基础认证方案
    ),
    # 分页
...
JWT_AUTH = {
    'JWT_EXPIRATION_DELTA': datetime.timedelta(days=1),  # 配置JWT token有效时长

    'JWT_RESPONSE_PAYLOAD_HANDLER': 'users.utils.jwt_response_payload_handler',  # 配置自定义JWT登陆返回数据
}
...
# FastDFS
FDFS_BASE_URL = 'http://192.168.65.128:8888/'
FDFS_CLIENT_CONF = os.path.join(BASE_DIR, 'utils/fastdfs/client.conf')

# django文件存储
DEFAULT_FILE_STORAGE = 'meiduo_mall.utils.fastdfs.fafs_storage.FastDFSStorage'
...
CRONJOBS = [
    ...
    ('*/1 * * * *', 'contents.crons.generate_static_index_html',
     '>> /home/python/Desktop/meiduo_mall/meiduo_mall/logs/crontab.log')
]
...
HAYSTACK_CONNECTIONS = {
    'default': {
        ...
        'URL': 'http://192.168.65.128:9200/',  # 此处为elasticsearch运行的服务器ip地址，端口号固定为9200
